...
class Parser(ParserInterface):
    def __init__(self, file_data):
        ...
        self.instance_scene = root.find("./collada:scene", NAMESPACES).find(
            "collada:instance_visual_scene", NAMESPACES
        )
    ...
    def find_libraries(self, root):
        ...
        self.library_controllers = root.find(
            "./collada:library_controllers", NAMESPACES
        )
    ...
    def parse(self):
        ...
        scene = self.library_scenes.find(
            f'collada:visual_scene[@id="{scene_url}"]', NAMESPACES
        )
    ...
    def parse_materials(self):
        for material in self.library_materials:
            ...
            if instance_effect is not None:
                ...
                effect = self.library_effects.find(
                    f'collada:effect[@id="{effect_url}"]', NAMESPACES
                )
                ...
                if effect is not None:
                    ...
                    self.scene.add_material(
                        Material(
                            name=material_name, shader="shader/uber.vsh", effect=effect
                        )
                    )
    ...
    def parse_node(self, xml_nodes: list, parent: str = None):
        for xml_node in xml_nodes:
            ...
            node: Node = Node(name=xml_node.attrib["name"], parent=parent)

            instance_geometry = xml_node.findall(
                "collada:instance_geometry", NAMESPACES
            )
            instance_controller = xml_node.findall(
                "collada:instance_controller", NAMESPACES
            )

            for xml_instance in [*instance_geometry, *instance_controller]:
                if instance_geometry:
                    instance = Node.Instance(
                        name=xml_instance.attrib["url"][1:], instance_type="GEOM"
                    )
                elif instance_controller:
                    instance = Node.Instance(
                        name=xml_instance.attrib["url"][1:], instance_type="CONT"
                    )
                else:
                    continue
                ...
                for instance_material in technique_common:
                    instance.add_bind(
                        instance_material.attrib["symbol"],
                        instance_material.attrib["target"][1:],
                    )
            ...
            if xml_matrix:
                ...
                matrix = [
                    [float(value) for value in matrix[x : x + 4]]
            ...
            self.parse_node(
                xml_node.findall("collada:node", NAMESPACES), node.get_name()
            )
        ...
        for node_index in range(len(nodes)):
            ...
            for instance in node.get_instances():
                if instance.get_type() == "CONT":
                    controller = self.library_controllers.find(
                        f'collada:controller[@id="{instance.get_name()}"]', NAMESPACES
                    )
                    geometry_url = controller[0].attrib["source"][1:]
                    collada_geometry = self.library_geometries.find(
                        f'collada:geometry[@id="{geometry_url}"]', NAMESPACES
                    )
                elif instance.get_type() == "GEOM":
                    collada_geometry = self.library_geometries.find(
                        f'collada:geometry[@id="{instance.get_name()}"]', NAMESPACES
                    )
                    controller = None
                else:
    ...
    def parse_controller(collada_controller, geometry: Geometry):
        ...
        for _input in joint_inputs:
            ...
            accessor = source.find(
                "collada:technique_common/collada:accessor", NAMESPACES
            )
            accessor_stride = int(accessor.attrib["stride"])
            accessor_source_url = accessor.attrib["source"]
            accessor_source = source.find(
                f'collada:*[@id="{accessor_source_url[1:]}"]', NAMESPACES
            )
            ...
            for param in params:
                ...
                if param_name == "JOINT":
                    for name in source_data:
                        geometry.add_joint(Geometry.Joint(name, Matrix4x4().matrix))

                if param_name == "TRANSFORM":
                    for x in range(
                        int(accessor_source.attrib["count"]) // int(accessor_stride)
                    ):
                        matrix = []
                        for y in source_data[
                            x * accessor_stride : (x + 1) * accessor_stride
                        ]:
            ...
            if semantic == "WEIGHT":
                ...
                for count in map(int, vcount.split()):
                    for i in range(count):
                        ...
                        geometry.add_weight(
                            Geometry.Weight(joint_index, weights[strength_index])
                        )
    ...
    def parse_geometry(self, collada_geometry) -> Geometry:
        ...
        for triangle in triangles:
            ...
            triangles = [
                [
                    triangles_temp[
                        polygon_index + point_index : polygon_index + point_index + 3
                    ]
