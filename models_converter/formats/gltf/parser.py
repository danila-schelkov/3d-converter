...
from models_converter.formats.gltf import Mesh
...
def _wrap_triangles(triangles, position_offset, normal_offset, texcoord_offset):
    return [
        [
            [
            ...
            for point in triangles[triangle_index : triangle_index + 3]
        ]
...
class Parser(ParserInterface):
    def __init__(self, data: bytes):
        ...
        self._json_chunk: GlTFChunk | None = None
        self._bin_chunk: GlTFChunk | None = None
    ...
    def _parse_bin(self):
        ...
        for accessor in self._gltf.accessors:
            ...
            types = {
                5120: (reader.read_int8, 1),
                5121: (reader.read_unsigned_int8, 1),
                5122: (reader.read_int16, 2),
                5123: (reader.read_unsigned_int16, 2),
                5125: (reader.read_unsigned_int32, 4),
                5126: (reader.read_float, 4),
            }

            if accessor.normalized:
                types = {
                    5120: (lambda: max(reader.read_int8() / 127, -1.0), 1),
                    5121: (lambda: reader.read_unsigned_int8() / 255, 1),
                    5122: (lambda: max(reader.read_int16() / 32767, -1.0), 2),
                    5123: (lambda: reader.read_unsigned_int16() / 65535, 2),
                    5125: (reader.read_unsigned_int32, 4),
                    5126: (reader.read_float, 4),
                }
            ...
            stride = (
                self._gltf.buffer_views[accessor.buffer_view].byte_stride
                or default_stride
            )
    ...
    def parse(self):
        ...
        if magic != b"glTF":
            raise TypeError(
                'Wrong file magic! "676c5446" expected, but given is ' + magic.hex()
            )

        self._version = reader.read_unsigned_int32()
        self._length = reader.read_unsigned_int32()

        self._json_chunk = GlTFChunk()
        self._bin_chunk = GlTFChunk()

        self._json_chunk.chunk_length = reader.read_unsigned_int32()
        self._json_chunk.chunk_name = reader.read(4)
        self._json_chunk.data = reader.read(self._json_chunk.chunk_length)

        self._bin_chunk.chunk_length = reader.read_unsigned_int32()
    ...
    def _parse_materials(self):
        ...
        for material in self._gltf.materials:
            if (
                self._gltf.is_using_extension("SC_shader")
                and material.extensions
                and "SC_shader" in material.extensions
            ):
                sc_shader = material.extensions
                material_name = sc_shader["name"]
            else:
                material_name = material["name"]

            self.scene.add_material(
                Material(
                    name=material_name,
    ...
    def _parse_node(self, gltf_node: Node, parent: str = None):
        ...
        node = universal.Node(name=node_name, parent=parent)
        ...
        if gltf_node.mesh is not None and type(self._gltf.meshes) is list:
            ...
            if gltf_node.skin is not None:
                ...
                instance = universal.Node.Instance(
                    name=geometry.get_name(), instance_type="CONT"
                )
                ...
                controller_bind_matrix = (
                    translation_matrix @ rotation_matrix @ scale_matrix
                )

                geometry.set_controller_bind_matrix(
                    controller_bind_matrix.get_linear_matrix()
                )
                ...
                inverse_controller_bind_matrix = (
                    translation_matrix @ rotation_matrix @ scale_matrix
                )
                ...
                for bind_matrix in bind_matrices_accessor:
                    ...
                    bind_matrices.append(
                        (
                            bind_matrix_ @ inverse_controller_bind_matrix
                        ).get_linear_matrix()
                    )
                ...
                instance = universal.Node.Instance(
                    name=geometry.get_name(), instance_type="GEOM"
                )
            ...
            for primitive in mesh.primitives:
                if primitive.to_dict() != {}:
                    ...
                    if primitive.material is not None:
                        material_name = self.scene.get_materials()[
                            material_id
                        ].get_name()
                    ...
                    for attribute_id in attributes:
                        ...
                        if attribute_id == "POSITION":
                            ...
                            points = [
                                [item[0] / 4096, 1 - item[1] / 4096]
                                for item in texcoord
                            ]
                        elif attribute_id.startswith("JOINTS"):
                            joints = self._accessors[attribute]
                        elif attribute_id.startswith("WEIGHTS"):
                            weights = self._accessors[attribute]

                            for joint_index, joint in enumerate(joints):
                                geometry.add_weight(
                                    Geometry.Weight(joint[0], weights[joint_index][0])
                                )
                                geometry.add_weight(
                                    Geometry.Weight(joint[1], weights[joint_index][1])
                                )
                                geometry.add_weight(
                                    Geometry.Weight(joint[2], weights[joint_index][2])
                                )
                                geometry.add_weight(
                                    Geometry.Weight(joint[3], weights[joint_index][3])
                                )
                    ...
                    triangles = _wrap_triangles(
                        triangles, position_offset, normal_offset, texcoord_offset
                    )

                    geometry.add_primitive(
                        Geometry.Primitive(material_name, triangles, input_vertices)
                    )
        ...
        node.add_frame(universal.Node.Frame(0, translation, scale, rotation))
