...
class Parser(ParserInterface):
    def __init__(self, file_data: bytes | str):
        ...
        self.lines: list[str] = file_data.splitlines()
    ...
    def parse(self):
        ...
        vertices_offsets = {"POSITION": 0, "TEXCOORD": 0, "NORMAL": 0}
        ...
        for line_index in range(len(self.lines)):
            ...
            if line.startswith("v "):  # POSITION
                ...
                for item in items:
                    ...
                    temp_list.append(
                        [
                            second_temp_list[0] - vertices_offsets["POSITION"],
                ...
                if self.position_temp:
                    ...
                    for x in range(0, len(self.position_temp), 3):
                        self.position.append(
                            [
                                vertex / position_scale
                                for vertex in self.position_temp[x : x + 3]
                            ]
                        )
                ...
                if self.normals_temp:
                    ...
                    for x in range(0, len(self.normals_temp), 3):
                        self.normals.append(
                            [
                                vertex / normals_scale
                                for vertex in self.normals_temp[x : x + 3]
                            ]
                        )
                ...
                if self.texcoord_temp:
                    ...
                    for x in range(0, len(self.texcoord_temp), 2):
                        self.texcoord.append(
                            [
                                vertex / texcoord_scale
                                for vertex in self.texcoord_temp[x : x + 2]
                            ]
                        )

            if (
                not line.startswith("f ")
                and triangles
                and geometry_name
                and self.position
                and self.normals
                ...
                if (
                    len(names) > len(self.scene.get_geometries()) + 1
                    and names[len(self.scene.get_geometries()) + 1] != geometry_name
                ):
                ...
                if not (
                    self.scene.get_geometries()
                    and self.scene.get_geometries()[-1].get_name() == geometry_name
                ):
                ...
                self.scene.get_geometries()[-1].add_primitive(
                    Geometry.Primitive(
                        material_name,
                        triangles,
                        self.scene.get_geometries()[-1].get_vertices(),
                    )
                )
        ...
        for geometry in self.scene.get_geometries():
            ...
            for primitive in geometry.get_primitives():
                instance.add_bind(
                    primitive.get_material_name(), primitive.get_material_name()
                )
