...
class Writer(WriterInterface):
    def __init__(self):
        ...
        self._temp_vertices_offsets = {"POSITION": 0, "TEXCOORD": 0, "NORMAL": 0}

        self._vertices_offsets = {"POSITION": 0, "TEXCOORD": 0, "NORMAL": 0}
    ...
    def write(self, scene: Scene):
        for geometry in scene.get_geometries():
            ...
            for vertex in geometry.get_vertices():
                ...
                self._temp_vertices_offsets[vertex.get_type()] += len(
                    vertex.get_points()
                )
            ...
            for primitive in geometry.get_primitives():
                self._lines.append(
                    f"o {geometry.get_name()}|{primitive.get_material_name()}\n"
                )
                for triangle in primitive.get_triangles():
                    ...
                    for point in triangle:
                        temp_list = [
                            str(
                                point[0] + self._vertices_offsets["POSITION"] + 1
                            ),  # POSITION
                            str(
                                point[2] + self._vertices_offsets["TEXCOORD"] + 1
                            ),  # TEXCOORD
                            str(
                                point[1] + self._vertices_offsets["NORMAL"] + 1
                            ),  # NORMAL
                        ]
