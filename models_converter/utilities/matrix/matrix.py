class Matrix:
    def __init__(self, **kwargs):
        if "matrix" in kwargs:
            ...
            matrix = self.get_identity_matrix(self.size)
        else:
            raise ValueError("No matrix parameters are provided.")

        self.size = (len(matrix[0]), len(matrix))  # x  # y

        self.matrix = matrix

    def __matmul__(self, other):
        if len(self.matrix) != len(other.matrix[0]) or len(self.matrix[0]) != len(
            other.matrix
        ):
    ...
    def transpose(self):
        ...
        self.matrix = [
            [self.matrix[row][col] for row in range(height)] for col in range(width)
        ]
