    ...
    def __init__(self, data: dict):
        ...
        for geometry_data in data['geometries']:
            ...
            for vertex_data in geometry_data['vertices']:
                ...
                write_source(
                    ...
                    source_name,
                    'float_array',
                    [' '.join([str(sub_item) for sub_item in item]) for item in vertex],
                    stride,
                    params
                )
            ...
            SubElement(vertices, 'input',
                       semantic='POSITION',
                       source=f'{geometry_name}-position')
            ...
            for material in geometry_data['materials']:
                ...
                for vertex in geometry_data['vertices']:
                    ...
                    if vertex_type == 'POSITION':
                        vertex_type = 'VERTEX'
                    source_url = f'#{geometry_name}-{vertex_type.lower()}'
                    if vertex_type == 'VERTEX':
                        source_url = f'#{geometry_name}-vertices'

                    SubElement(triangles, 'input',
                               semantic=vertex_type,
                               source=source_url,
                               offset=f'{vertex_index}')
            ...
            if geometry_data['have_bind_matrix']:
                ...
                SubElement(joints, 'input',
                           semantic='JOINT',
                           source=f'#{joints_names_source_id}')
                SubElement(joints, 'input',
                           semantic='INV_BIND_MATRIX',
                           source=f'#{joints_matrices_source_id}')
                ...
                SubElement(vertex_weights, 'input',
                           semantic='JOINT',
                           source=f'#{joints_names_source_id}',
                           offset='0')
                SubElement(vertex_weights, 'input',
        ...
        for node_data in data['nodes']:
            ...
            if node_data['has_target']:
                ...
                if target_type == 'CONT':
                    instance_controller = SubElement(node, 'instance_controller', url=f'#{target_id}')
